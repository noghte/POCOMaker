...
ORDER BY FK.TABLE_NAME,
        FK.COLUMN_NAME";
            ...
            public override void ProcessForeignKeys(List<ForeignKey> fkList, Tables tables, bool useCamelCase, bool prependSchemaName, string collectionType)
            {
                ...
                foreach (var constraint in constraints)
                {
                    ...
                    string pkPropName = fkTable.GetMyUniqueColumnPropertyName(pkTableHumanCase, foreignKeys);
                    string fkPropName = pkTable.GetMyUniqueColumnPropertyName(fkTable.NameHumanCase, foreignKeys);
                }
        ...
        public class Table
        {
            ...
            public List<string> MyReverseNavigationUniquePropName;
            ...
            public Table()
            {
                ...
                MyReverseNavigationUniquePropName = new List<string>();
            }
            ...
            {
                string col;
                string fk = fKeys[0].FkColumn;
                if (MyReverseNavigationUniquePropName.Count(p => p.Contains(columnName + "!")) > 1)
                    col = columnName + "_" + fk;
                else
                    col = columnName;
            ...
            public string GetMyUniqueColumnPropertyName(string columnName, IList<ForeignKey> fKeys)
            {
                if (MyReverseNavigationUniquePropName.Count == 0)
                {
                    MyReverseNavigationUniquePropName.Add(NameHumanCase);
                    MyReverseNavigationUniquePropName.AddRange(Columns.Select(c => c.PropertyNameHumanCase));
                }
                if (!MyReverseNavigationUniquePropName.Contains(columnName + "!"))
                {
                    MyReverseNavigationUniquePropName.Add(columnName + "!");
                    return columnName + "!";
                }
                for (int n = 1; n < 100; ++n)
                {
                    string col = columnName + "!" + n;
                    //if (MyReverseNavigationUniquePropName.Contains(col))
                    //    continue;
                    MyReverseNavigationUniquePropName.Add(col);
                    return col;
                }
            ...
            public void IdentifyMappingTable(List<ForeignKey> fkList, Tables tables, bool useCamelCase, bool prependSchemaName, string collectionType)
            {
                ...
                rightTable.AddReverseNavigation(Relationship.ManyToMany, rightTable.NameHumanCase, leftTable,
                                               rightTable.GetUniqueColumnPropertyName(leftTable.NameHumanCase, foreignKeys), null, collectionType);

                leftTable.AddReverseNavigation(Relationship.ManyToMany, leftTable.NameHumanCase, rightTable,
                                                leftTable.GetUniqueColumnPropertyName(rightTable.NameHumanCase, foreignKeys), null, collectionType);
            }
        }
